...
const DEFAULT_TEMPLATE = `עברה כשנה מאז השתתפותך במחקר המיקרוביום כחלק מבדיקות הסקר בשיבא.
...
export default function App() {
  ...
  const [templateText, setTemplateText] = useState<string>(() => {
    try {
      return localStorage.getItem("sheba_whatsapp_template_v2") || CLINICAL_TEMPLATES.find(t => t.id === "std_11m")?.text || DEFAULT_TEMPLATE;
    } catch (e) {
      console.error("Error accessing localStorage", e);
      return CLINICAL_TEMPLATES.find(t => t.id === "std_11m")?.text || DEFAULT_TEMPLATE;
    }
  });
